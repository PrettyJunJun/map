...
class Commodity extends Admin
{
    ...
    public function commodityy(){ //修改井盖信息
        $model = model('WellType');
        $data = $model->select();
        $this->assign('data',$data);
        $sm = input('');
    ...
    public function lists(){  //查出井盖类型列表
        $model = model('WellType');
    ...
    public function type(){  //添加井盖类型属性值
        if (request()->isPost()){
            $data = input('');
            $model = model('WellType');
    ...
    public function delete(){ //删除井盖类型相对应的井盖列表也将删除
        if (request()->isAjax()) {
