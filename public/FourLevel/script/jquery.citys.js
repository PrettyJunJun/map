...
;(function (factory) {
    ...
    $.fn.citys = function (parameter, getApi) {
        ...
        var defaults = {
            ...
            //>>是否必须选一个
            required: true,
            //>>当无数据时的表现形式:'hidden'隐藏,'disabled'禁用,为空不做任何处理
            nodata: 'hidden',
            onChange: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $province.on('change', function () {
                        //>>选中节点的区划代码
                        options.province = $(this).find('option:selected').data('code') || 0;
                        options.city = 0;
                        options.area = 0;
                        updateData();
                        format.city();
                    ...
                    $city.on('change', function () {
                        //>>选中节点的区划代码
                        options.city = $(this).find('option:selected').data('code') || 0;
                        options.area = 0;
                        updateData();
                        format.area();
                        options.onChange(_api.getInfo());
                    ...
                    $area.on('change', function () {
                        //>>选中节点的区划代码
                        options.area = $(this).find('option:selected').data('code') || 0;
                        options.onChange(_api.getInfo());
                    });
                    //>>初始化
                    updateData();
